...
```bash
# Find in-progress tasks in SESSION-TASKS.md
grep -B 2 -A 5 "\[~\]\|\[ \] .*in progress" SESSION-TASKS.md 2>/dev/null || echo "No in-progress tasks marked"

# Alternative: look for tasks being worked on
grep -B 3 "- \[ \]" SESSION-TASKS.md | head -20
```
...
[If found in SESSION-TASKS.md:]
From SESSION-TASKS.md:
...
Active tasks in SESSION-TASKS.md:
...
Next task: [Specific next action from SESSION-TASKS.md]
...
⚠️ Task marked in progress but no recent activity
   Review task status in SESSION-TASKS.md
